...
const commentLikeRepository = dataSource.getRepository(
  commentLikesEntity
)
...
export function sendPostComment(
  ...
  comment.content = encode(comment.content)
...
export function deleteCommentRelation(
  ...
  return new Promise((resolve, reject) => {
    ...
    postCommentRepository
      ...
      .then(result => {
        console.info(
          `[${FgYellow}delete no relation PostCmment${Reset}]`
        )
        console.log(result)
      })
      .catch(err => {
  ...
  commentRepository
    ...
    .then(comments => {
      ...
      const replys = comments.filter(c => {
        return (
          c.replyTo !== null && !commentsId.includes(c.replyTo)
        )
      })
      console.info(
        `[${FgYellow}delete no relation comment${Reset}]`
      )
      console.log(replys)
      Promise.all(
        replys.map(c => deleteCommentRelation(c.id, c.publisher))
      )
        .then(result => {
          console.log(result)
        })
