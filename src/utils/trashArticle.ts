...
const 前面垫话 = ['曾经说过', '在不经意间这样说过', '曾经提到过', '说过一句富有哲理的话']

const 初始主题 = ['一天掉多少根头发', '中午吃什么', '学生会退会', '好好学习', '生活的意义', '科学和人文谁更有意义']
...
function 生成文章(theme?: string) {
  ...
  const 排版 = '<div>' + 文章.join('</div><div>') + '</div>'
  return 排版
}
export function getTestArticle(theme?: string) {
  const title = 随便取一句(theme ?? 初始主题)
  return {
    title,
    content: 生成文章(title)
  }
