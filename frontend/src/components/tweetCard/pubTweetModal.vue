<template>
  ...
  footer: 'soft'
    ...
    <template #default>
      <n-space vertical>
        <n-input v-model:value="title" type="text" :placeholder="$t('input_tweet_title.name')"></n-input>
        ...
        <n-divider style="margin: 5px 0;" title-placement="left">
          <n-el style="margin-right: 10px;">{{ $t('upload_tweet_image.name') }}</n-el>
    ...
    <template #footer>
      <n-space>
        <n-button :loading="loading" :disabled="loading" @click="submit" type="primary">{{ $t('submit.name') }}</n-button>
