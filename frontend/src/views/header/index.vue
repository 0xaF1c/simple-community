<template>
  <n-space justify="space-between" align="center" :style="{
    height: `100%`,
    padding: `0 ${headerPadding}px`
  }">
    <n-el class="left" style="left: 20px;">
      <n-avatar src="/logo.svg" style="background-color: #0000; cursor: pointer;"></n-avatar>
    </n-el>
    ...
    <n-el class="right">
      <n-space item-style="font-size: 17px;" align="center">
        ...
        <avatar-link :userData="userData" style="display: flex; justify-items: center; align-items: center;"
          :size="30"></avatar-link>
        <!-- <n-el>
          <span style="font-size: 1.3rem;">{{ userData?.name }}</span>
          <n-button style="font-size: 1rem;" text>@{{ userData?.account }}</n-button>
        </n-el> -->
        <n-button align="center" v-if="userData === null" quaternary @click="showLoginModal()">
